...
export class TasksService {
  ...
  async create(user: User, createTaskDto: CreateTaskDto) {
    ...
    return this.prismaService.$transaction(async (prisma) => {
      ...
      if (houseUsers.length !== createTaskDto.user_ids.length) {
        throw new NotFoundException('Some users are not associated with the house');
      }
      ...
      const taskDetail: TaskDetailDto = {
        ...
        recurringTask: detailedTask.recurringTask ? detailedTask.recurringTask : null,
  ...
  async update(id: number, updateTaskDto: UpdateTaskDto): Promise<TaskDetailDto> {
    ...
    const existingHouseUserIds = existingTask.taskSplits.map((split) => split.house_user_id);
    
    const houseUserIdsToAdd = houseUserIds.filter((id) => !existingHouseUserIds.includes(id));
    const houseUserIdsToRemove = existingHouseUserIds.filter((id) => !houseUserIds.includes(id));
  ...
  async updateTaskSplitStatus(
    ...
    return await this.prismaService.$transaction(async (prisma) => {
      ...
      if (taskSplit.task_id !== task_id || taskSplit.houseUser.house_id !== house_id) {
        throw new ForbiddenException('This TaskSplit does not belong to the specified task or house');
      }
  ...
  async createTaskCategory(user: any, createTaskCategoryDto: CreateTaskCategoryDto): Promise<TaskCategoryDetailDto> {
    return this.prismaService.$transaction(async (prisma) => {
      ...
      if (!houseUser) {
        throw new NotFoundException('HouseUser not found for the given house and user.');
      }
  ...
  async updateTaskCategory(user: User, id: number, updateTaskCategoryDto: UpdateTaskCategoryDto) {
    ...
    // Fetch the detailed task category including relations within the same transaction
    const detailedTaskCategory = await this.prismaService.taskCategory.findUnique({
